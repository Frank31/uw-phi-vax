---
title: "07_validate_index_third_version"
...
# subset columns
full_data <- full_data %>% select(location, region, year, gbd_location_id, iso_code, iso_num_code, result, prop_val_MCV1, prop_val_DTP1, prop_val_DTP3, prop_val_MCV2) %>% filter(year!=2020)
...
# fit a model for vaccines in the complete vaccination series
modeldtp <- glm(prop_val_DTP3~factor(super_region) + year + result, data=train, family = "binomial")
summary(modeldtp)
par(mfrow=c(2,2))
plot(modeldtp)

# predict vaccination levels using the test data
pred1 <- predict(modeldtp, newdata = test, type = "response")

par(mfrow=c(1,1))
plot(pred1, test$prop_val_DTP3,
     xlab='Predicted Vaccination Coverage',
     ylab='Actual Vaccination Coverage',
     main='Predicted vs. Actual Values DTP3, 1990-2018')
abline(a=0, b=1)

# calculate Brier Score: to measure accuracy of probabilistic predictions
BrierScore(test$prop_val_DTP3, pred1)

# fit a second model for measles: MCV2 contained a lot of missing data
modelmcv <- glm(prop_val_MCV1~factor(super_region) + year + result, data=train, family = "binomial")
summary(modelmcv)
par(mfrow=c(2,2))
plot(modelmcv)


# predict the actual vaccination levels using the testing data
pred2 <- predict(modelmcv, newdata = test, type = "response")

par(mfrow=c(1,1))
plot(pred2, test$prop_val_MCV1,
     xlab = "Predicted Vaccination Coverage",
     ylab = "Actual Vaccination Coverage", 
     main = "Predicted vs. Actual Values MCV1, 1990-2018")

# calculate the Brier Score: to measure accuracy of probabilistic predictions
BrierScore(test$prop_val_MCV1, pred2)
...
pred2019dtp3 <- predict(modeldtp, newdata=newdata, type = "response")
...
BrierScore(newdata$prop_val_DTP3, pred2019dtp3)

pred2019mcv <- predict(modelmcv, newdata = newdata, type = "response")
...
# calculate the Brier Score
BrierScore(newdata$prop_val_MCV1, pred2019dtp3)
