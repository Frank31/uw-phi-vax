...
# load data sets
dt1 <- as.data.table(readRDS(file=paste0(prepped_data_dir, "aim_2/17_merged_dataset_third_version.RDS"))) # merged data set with missing values
dt2 <- as.data.table(readRDS(file=paste0(prepped_data_dir, "aim_2/18_prepped_data_for_analysis_third_version.RDS"))) # prepped data set with full series and dropped rows
...
histograms2 = lapply(indexVars, function(v) {
  ...
  ggplot(plotdt2[variable==v], aes(value)) +
    geom_histogram() +
    labs(title = paste('Histogram of Post-transformation', l), y = 'Value', x = l,
         caption='Variables are post-imputation.') +
    theme_minimal()
})

# Create PDF File with relevant visuals
outputFile11 <- paste0(visDir, "/aim_2/third_version/03_histograms_of_data.PDF")
...
outputFile11b <- paste0(visDir, "aim_2/third_version/04_correlation_matrix.PDF")
pdf(outputFile11b, height=11, width=11)
corrplot
dev.off()

# view summary statistics for all variables
st(dt1, file=paste0(visDir, "aim_2/third_version/05_descriptive_stats_merged_data.PDF"))
st(dt2, file=paste0(visDir, "aim_2/third_version/06_descriptive_stats_extrapolated_data.PDF"))
...
write.csv(miss_table_reformat, file=paste0(visDir, "aim_2/third_version/07_missingness_in_merged_data.csv"))
...
write.csv(dropped_locs, file = paste0(visDir, "aim_2/third_version/08_dropped_locations.csv"))
...
write.csv(skill_att_miss, file = paste0(visDir, "aim_2/third_version/09_locations_missing_skill_birth_attendants.csv"))
...
write.csv(miss_table_reformat2, file=paste0(visDir, "aim_2/third_version/10_missingness_in_merged_data_without_locations_dropped.csv"))
