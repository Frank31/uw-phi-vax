...
# use Model 1 (diphtheria) to predict vaccination coverage given new index value
pred2019dip <- predict(model1, newdata = worst_performer, type = "response")
...
# the best coverage predicted in 2019:  .893
# actual vaccination coverage in 2019:  .148
...
full_data$log_dalys_diphtheria_rate <- log(full_data$dalys_diphtheria_rate+1)

# predict new disease burden with new vaccination coverage levels in Somalia
model2 <- lm(formula = log_dalys_diphtheria_rate ~ prop_val_DTP3+location+year, data=full_data)

sink(paste0(file_folder, "/Results/aim_3/01_regression_dip_output.txt"))
...
ggplotRegression <- function (fit) {
  ...
  ggplot(fit$model, aes_string(x = names(fit$model)[2], y = names(fit$model)[1])) + 
    ...
    labs(title = paste("Adj R2 = ",signif(summary(fit)$adj.r.squared, 5),
         ...
         xlab = "Vaccination coverage (DTP3)",
         ylab = "Diphtheria DALYs")
}
...
somalia_data <- with(full_data, data.frame(location = rep("Somalia", each=1, length.out=6),
                                      ...
                                      prop_val_DTP3 = seq(from=.245, to=.903, by=.1316)))

# create new data that has different levels of vaccination coverage
full_data_subset <- full_data %>% rename(orig_prop_val_DTP3=prop_val_DTP3)
...
pred2019dipdalys <- predict(model2, newdata = somalia_newdata, type = "response")
...
table$predicted_dip_log_dalys <- NA
for (i in 1:6) {
  table$predicted_dip_log_dalys[i] <- pred2019dipdalys[i]
}

# transform the predicted values so they are not on the log scale
table$predicted_dip_dalys <- exp(table$predicted_dip_log_dalys) - 1

# save table of predicted disease burden under different levels of vaccination coverage
write.csv(table, file=paste0(resDir, "aim_3/02_predicted_diphtheria_counterfactual_results.csv"))
